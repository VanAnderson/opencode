...
When you attach to a backend started by `opencode web`, the TUI and browser app share the same session state, including queued messages, steer interruptions, and blocked pending items.
...
The interactive browser UI served by this command is the main OpenCode app. If you later use `opencode attach` against the same backend, the TUI joins the same live sessions and queue state instead of creating a separate local queue.
